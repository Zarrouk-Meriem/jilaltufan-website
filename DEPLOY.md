...
**Migrating Neon after a schema change** (rule since 2026-09-19: from this machine, _before_ the push — the build never migrates). Payload's own `migrate` prompts about the leftover `dev` row and hangs without a terminal, so apply the new migration's `up` SQL with psql in its idempotent form and record the row, e.g. for `20260922_075008_status_emails`:

```bash
docker run --rm postgres:17-alpine psql "$(grep ^DATABASE_URL= .env.vercel | cut -d= -f2- | tr -d '"')" -v ON_ERROR_STOP=1 -c 'ALTER TABLE "applications" ADD COLUMN IF NOT EXISTS "send_rejection_email" boolean DEFAULT false;' -c 'ALTER TABLE "applications" ADD COLUMN IF NOT EXISTS "reviewing_email_sent_at" timestamp(3) with time zone;' -c 'ALTER TABLE "applications" ADD COLUMN IF NOT EXISTS "waitlist_email_sent_at" timestamp(3) with time zone;' -c 'ALTER TABLE "applications" ADD COLUMN IF NOT EXISTS "rejection_email_sent_at" timestamp(3) with time zone;' -c "INSERT INTO payload_migrations (name, batch, created_at, updated_at) SELECT '20260922_075008_status_emails', (SELECT coalesce(max(batch),0)+1 FROM payload_migrations), now(), now() WHERE NOT EXISTS (SELECT 1 FROM payload_migrations WHERE name='20260922_075008_status_emails');" -c "SELECT name, batch FROM payload_migrations ORDER BY id"
```
...
And for `20260922_124615_activity_log` (the activity log, 2026-09-22 — a new table, two enums, and a column on the locks table; run after the one above):

```bash
docker run --rm postgres:17-alpine psql "$(grep ^DATABASE_URL= .env.vercel | cut -d= -f2- | tr -d '"')" -v ON_ERROR_STOP=1 -c "DO \$\$ BEGIN CREATE TYPE \"public\".\"enum_activity_action\" AS ENUM('create', 'update', 'delete'); EXCEPTION WHEN duplicate_object THEN NULL; END \$\$;" -c "DO \$\$ BEGIN CREATE TYPE \"public\".\"enum_activity_target\" AS ENUM('programs', 'sessions', 'instructors', 'projects', 'minbar-posts', 'materials', 'events', 'applications', 'application-files', 'contact-messages', 'users', 'media', 'about-page', 'home-page', 'students-page', 'instructors-page', 'site-settings', 'navigation', 'footer'); EXCEPTION WHEN duplicate_object THEN NULL; END \$\$;" -c 'CREATE TABLE IF NOT EXISTS "activity" ("id" serial PRIMARY KEY NOT NULL, "created_at" timestamp(3) with time zone DEFAULT now() NOT NULL, "action" "enum_activity_action" NOT NULL, "target" "enum_activity_target" NOT NULL, "title" varchar, "doc_id" varchar, "user_id" integer REFERENCES "public"."users"("id") ON DELETE set null, "user_email" varchar, "locale" varchar, "changes" jsonb, "updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL);' -c 'CREATE INDEX IF NOT EXISTS "activity_action_idx" ON "activity" ("action"); CREATE INDEX IF NOT EXISTS "activity_target_idx" ON "activity" ("target"); CREATE INDEX IF NOT EXISTS "activity_doc_id_idx" ON "activity" ("doc_id"); CREATE INDEX IF NOT EXISTS "activity_user_idx" ON "activity" ("user_id"); CREATE INDEX IF NOT EXISTS "activity_user_email_idx" ON "activity" ("user_email"); CREATE INDEX IF NOT EXISTS "activity_updated_at_idx" ON "activity" ("updated_at");' -c 'ALTER TABLE "payload_locked_documents_rels" ADD COLUMN IF NOT EXISTS "activity_id" integer REFERENCES "public"."activity"("id") ON DELETE cascade; CREATE INDEX IF NOT EXISTS "payload_locked_documents_rels_activity_id_idx" ON "payload_locked_documents_rels" ("activity_id");' -c "INSERT INTO payload_migrations (name, batch, created_at, updated_at) SELECT '20260922_124615_activity_log', (SELECT coalesce(max(batch),0)+1 FROM payload_migrations), now(), now() WHERE NOT EXISTS (SELECT 1 FROM payload_migrations WHERE name='20260922_124615_activity_log');" -c "SELECT name, batch FROM payload_migrations ORDER BY id"
```

And for `20260922_135917_activity_auth_actions` (logins in the activity log, 2026-09-22 — three enum values; each `ADD VALUE` must run as its own statement, outside a transaction, which psql `-c` does):

```bash
docker run --rm postgres:17-alpine psql "$(grep ^DATABASE_URL= .env.vercel | cut -d= -f2- | tr -d '"')" -v ON_ERROR_STOP=1 -c "ALTER TYPE \"public\".\"enum_activity_action\" ADD VALUE IF NOT EXISTS 'login';" -c "ALTER TYPE \"public\".\"enum_activity_action\" ADD VALUE IF NOT EXISTS 'login-failed';" -c "ALTER TYPE \"public\".\"enum_activity_action\" ADD VALUE IF NOT EXISTS 'logout';" -c "INSERT INTO payload_migrations (name, batch, created_at, updated_at) SELECT '20260922_135917_activity_auth_actions', (SELECT coalesce(max(batch),0)+1 FROM payload_migrations), now(), now() WHERE NOT EXISTS (SELECT 1 FROM payload_migrations WHERE name='20260922_135917_activity_auth_actions');" -c "SELECT name, batch FROM payload_migrations ORDER BY id"
```
...
Lighthouse (mobile, production build, M7): Home 94 / 100 / 100 / 100, program page 94 / 100 / 100 / 100 (performance / accessibility / best practices / SEO). Re-measured 2026-09-20 after the Noto Kufi swap: 90 / 100 / 100 / 100, and 92–93 / 100 / 100 / 100 with CLS 0 on all runs after the fixes recorded in TODO.md (static rendering restored, a 43 kB font subset, two layout shifts removed). Re-measure with `NEXT_DIST_DIR=.next-prod NEXT_PUBLIC_SITE_URL=http://localhost:3002 pnpm build`, `PORT=3002 pnpm start`, then `npx lighthouse http://localhost:3002/ar --preset=perf --form-factor=mobile --screenEmulation.mobile` (the site URL must match the port or the canonical audit fails).
