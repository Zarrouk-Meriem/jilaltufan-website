...
- **Every staff write is logged.** `src/lib/payload/activity.ts` wraps every collection and global in `payload.config.ts` with `afterChange`/`afterDelete` hooks that write a row to the admin-only `activity` collection (the «سجلّ النشاط» page): who, when, which document, which top-level fields changed (values only when short and the field has no read restriction). Only signed-in staff are recorded; a system follow-up write passes `context[SKIP_ACTIVITY]` (the status-email stamp does). New collections and globals are covered by being in the config lists — nothing to add — but the `target` enum is a schema change, so a new slug needs a migration. `tests/e2e/activity.spec.ts` needs the throwaway accounts from `scripts/e2e-staff.ts`.
