...
- **Texture is the mark cascade** (`pattern-marks-navy` / `pattern-marks-paper`, `src/styles/patterns.css`): the designer's arrow mark on a staggered lattice, outlined, faint. The keffiyeh net stays defined but unused. The gradient cascade is art, not texture: `LivingCascade` (`cascade-live` in `patterns.css`) is the outlined marks as a mask (a crisp stroke over a wide faint one: the glow is in the mask) over one slow-drifting red→blue→navy gradient layer, feathered by a radial mask — one placement per surface at most, lighter (`opacity-40`) over a photograph, the drift stops under reduced motion, and **no blur filter on a moving layer**: a blurred twin made every frame slow enough to lag scrolling by a step (header.spec caught it, 2026-09-21). The static file `public/brand/patterns/cascade.svg` is the same treatment for print or a one-off.
