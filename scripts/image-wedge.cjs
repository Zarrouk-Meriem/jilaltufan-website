 ...
 *
 * Exit 1 if any variant hangs. Clears .next/cache/images first so variants are cold.
...
async function main() {
  fs.rmSync(path.join(process.cwd(), '.next/cache/images'), { recursive: true, force: true })
  ...
  const worker = async (id) => {
    for (let i = 0; i < 12; i++) {
      ...
      page.goto(`${base}/ar${i % 2 ? '' : '/programs/palestine-our-compass'}`).catch(() => {})
  ...
  let hung = 0
  for (const img of ['logo-color', 'logo-on-dark']) {
    for (const w of [64, 96, 128, 256, 384]) {
      const url = `${base}/_next/image?url=%2Fbrand%2F${img}.png&w=${w}&q=75`
